...
- 最後核對日期：2026 年 9 月 21 日。
- 本次正式發布：經使用者同意，以 `b66fa3d` 統一日本中英文訂課五步驟與確認提醒，明確為 Accent 寄送 Invoice、核實全額入帳後私訊確認才完成訂課；Vercel 回報 success，已在原正式網址核對中英文內容。澳洲流程、價格、地點、版面及條款未修改。
- 外部 Google Form 的中英文開頭與送出後提示已儲存並重新讀取確認，學生預覽已顯示新說明；保留原本分組填表與回覆副本提醒，未改題目、半日課／Hanazono 選項、條款或管理表，未送出測試回覆。
- 本次驗證：型別檢查、修改檔案 ESLint、Next.js／vinext 正式建置與 7 項回歸測試通過；本機與正式瀏覽器已確認中英文訂課內容，正式頁面無瀏覽器錯誤；未檢查本次文字在手機／平板尺寸的排版，未完成 Vercel 伺服器端執行紀錄查詢。
- 本次發布：中英文 FAQ 改為價格價值第一、Hanazono 第二且第一題預設展開，中文雪場名稱及 APSI 雙板二級資格說明已以 `f2c4208` 發布，Vercel 回報 success；本機中英文驗證已完成，正式畫面尚未完成瀏覽器驗證。
- 中文 FAQ 與價目表的起點標示為 Hirafu（比羅夫）及 Annupuri（安努普利），英文介面維持英文名稱。
...
- 核對時的正式程式版本：`b66fa3d`（`Align Japan booking workflow with approved form instructions`）；透過 GitHub 連線發布，後續文件提交不變更網站程式。本機 HEAD 仍為 `61e8efa`，工作目錄保留已發布修改，下次 Git 操作前須先同步遠端，避免覆蓋新提交。
...
- 「我們是誰」中英文文案說明兩位都持有 APSI 單板三級與雙板二級資格，並能清楚解釋動作、技術與背後理論。
- 日本／澳洲訂課流程切換器，各自顯示 5 個步驟與確認提醒。
- 日本訂課流程已正式更新為查看價格、聯絡確認日期／程度／人數／地點、填表核對、收到 Accent Invoice 並付款、核實全額入帳後收到私訊確認。
...
以下流程已以 `b66fa3d` 發布至正式網站，與表單開頭及送出提示一致：

1. 客人先查看課程價格與包含項目。
2. 與 Ann／Lonny 確認日期、學生程度、人數與上課地點。
3. 填寫已討論的課程安排與學生資料，由 Ann／Lonny 核對。
4. Accent 寄送 Invoice，客人在帳單指定期限內付款；付款前教練與時段尚未正式保留。
5. Ann／Lonny 核實全額入帳後，透過私訊確認訂課成功。

網站提醒：填寫表單或收到 Invoice，不代表時段已正式保留；核實全額入帳並私訊確認後才完成訂課。外部 Google Form 的開頭及送出後提示已採用相同中英文流程。
