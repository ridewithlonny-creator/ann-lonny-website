...
- 核對時的正式程式版本：遠端 `main` 的 `f2c4208`（`Prioritize lesson value FAQ and clarify resort names and qualifications`）；透過 GitHub 連線發布。本機 HEAD 仍為 `61e8efa`，工作目錄保留已發布修改，下次 Git 操作前須先同步遠端，避免覆蓋新提交。
